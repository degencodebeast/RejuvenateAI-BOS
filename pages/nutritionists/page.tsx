...
import Header from "@app/components/nutritionist-header";
...
export default function NutritionistPage(){
  const toast=useToast({duration:3000,position:'top',status:'success',title:'Your appointment was booked successfully'})
...
setTimeout(()=>{
    ...
    return (
<Box className="bg-primaryYellowTrans" h={'100vh'} px={6} overflowY={'auto'} pb={12}>
  <Header bg="white"/>
      ...
      <Modal size={'3xl'} isOpen={isOpen} onClose={onClose}>
        ...
        <ModalContent>
          ...
          <ModalBody>
            ...
            <Text fontWeight={'semibold'} mb={2} className="text-primaryGreen"> Appointment Details:</Text>
...
<Flex gap={4} wrap={'wrap'} maxW={350}>
   ...
   return <Button onClick={()=>handleSectionDuration(dur)} key={'dur'+i} className={`text-primaryGreen rounded-full px-4 py-2 border ${sectionDuration===dur?'bg-primaryYellow':''}`}>
{dur} Mins
   </Button>
    })}
</Flex>
